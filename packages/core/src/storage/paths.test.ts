...
describe("constants", () => {
  it("KILOG_DIR_NAME should be .kilog", () => {
    expect(KILOG_DIR_NAME).toBe(".kilog");
  });

  it("RAW_DIR should be .kilog/raw", () => {
    expect(RAW_DIR).toBe(".kilog/raw");
  });

  it("INDEX_DIR should be .kilog/index", () => {
    expect(INDEX_DIR).toBe(".kilog/index");
  });
...
describe("dbFilePath", () => {
  it("should compose INDEX_DIR and DB_FILE", () => {
    ...
    expect(result).toBe("/project/.kilog/index/logs.duckdb");
  });
});
...
describe("rawFilePath", () => {
  it("should join baseDir, RAW_DIR, and filename", () => {
    ...
    expect(result).toBe("/project/.kilog/raw/2026-04-18.node.jsonl");
  });

  it("should handle baseDir without trailing slash", () => {
    const result = rawFilePath("/my/project", "2026-01-01", "browser");
    expect(result).toBe("/my/project/.kilog/raw/2026-01-01.browser.jsonl");
  });
  ...
  it("should handle baseDir with trailing slash consistently", () => {
    ...
    expect(a).toContain(".kilog/raw/2026-01-01.node.jsonl");
  });
});
