...
describe("reindex", () => {
  ...
  beforeEach(async () => {
    baseDir = await mkdtemp(path.join(tmpdir(), "kilog-indexer-"));
...
describe("indexFile", () => {
  ...
  beforeEach(async () => {
    baseDir = await mkdtemp(path.join(tmpdir(), "kilog-indexfile-"));
    await mkdir(path.join(baseDir, ".kilog", "raw"), { recursive: true });
    await mkdir(path.join(baseDir, ".kilog", "index"), { recursive: true });
  });
  ...
  it("should return count of indexed events from a single file", async () => {
    ...
    const filePath = path.join(baseDir, ".kilog", "raw", "test.jsonl");
  ...
  it("should return 0 for empty file", async () => {
    const filePath = path.join(baseDir, ".kilog", "raw", "empty.jsonl");
