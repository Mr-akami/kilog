...
Hono bundled by Vite via [`@hono/vite-dev-server`](https://github.com/honojs/vite-plugins/tree/main/packages/dev-server) + [`@mr-akami/kilog/vite-plugin`](../../docs/vite-plugin.md). Demonstrates that `kilogPlugin()` captures **server-side** `console` / `fetch` / errors when Hono runs inside the Vite dev process — no `--import @mr-akami/kilog/register` needed.
